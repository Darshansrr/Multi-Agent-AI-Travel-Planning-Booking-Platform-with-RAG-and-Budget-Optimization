...
st.set_page_config(
    page_title="Multi-Agent AI Travel Planning & Booking Platform",
    page_icon="🚀",
    layout="wide"
)
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("<div class='sidebar-title'>🌍 Multi-Agent AI Travel Planning & Booking Platform</div>", unsafe_allow_html=True)
    st.markdown("---")

    thread_id = st.text_input("👤 User ID", value="Darshan_user",
    ...
    for step in ["① Flight Agent", "② Hotel Agent", "③ Budget Agent ", "④Rag Agent"," ⑤ Itinerary Agent","⑥ Final Agent"]:
...
<div class="hero-wrapper">
    ...
    <div class="hero-content">
        ...
        <div class="hero-title">🚀 Multi-Agent AI Travel Planning & Booking Platform with RAG</div>
        <div class="hero-sub">Six specialized agents work together — searching flights, hotels,optimizing budget,rag travel guides, building an itinerary, and delivering your perfect trip plan.</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
if generate:
    if not user_query.strip():
        ...
        collected = {"flight_results": "", "hotel_results": "","budget_analysis":"", "rag_context": "",
            ...
            for node_name, state_update in chunk.items():
                ...
                with st.status(f"{icon}  {label}", state="complete", expanded=True):
                    if node_name == "flight_agent":
                        ...
                        text = state_update.get("budget_analysis", "")
                        collected["budget_analysis"] = text
                        st.markdown(text or "_No budget analysis generated._")
                    

                    elif node_name == "rag_agent":
                        text = state_update.get("rag_context", "")
                        collected["rag_context"] = text
                        st.markdown(text or "_No RAG context found._")
...
## 💰 Budget Analysis
{collected['budget_analysis'] or 'N/A'}
---

## 📚 Rag Context
{collected['rag_context'] or 'N/A'}
